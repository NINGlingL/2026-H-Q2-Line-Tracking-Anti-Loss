...
def position_from_pixel(
    ...
    # 透视校正：远端被压缩，需要非线性校正
    # ratio=0 → 左端, ratio=0.5 → 中心, ratio=1 → 右端
    # 右端压缩约15%，使用二次函数校正
    corrected_ratio = ratio + 0.15 * ratio * (ratio - 0.5) * 2
    corrected_ratio = max(0.0, min(1.0, corrected_ratio))

    position_cm = start_position_cm + corrected_ratio * (
...
# --- MaixCAM Pro 补光灯控制 ---
USE_LED_LIGHT = True  # 是否开启补光灯（仅MaixCAM Pro有效）

# 补光模式参数优化
if USE_LED_LIGHT:
    EXPOSURE_US_WITH_LIGHT = 800        # 补光时曝光时间（降低，减少运动模糊）
    ANALOG_GAIN_DB_WITH_LIGHT = 0       # 补光时增益（关闭，光照充足）
    DETECTION_CONF_STATIC_LIGHT = 0.45  # 补光时静止阈值（提高，图像质量更好）
    DETECTION_CONF_FAST_LIGHT = 0.22    # 补光时快速阈值（提高）
else:
    EXPOSURE_US_WITH_LIGHT = 1200
    ANALOG_GAIN_DB_WITH_LIGHT = 3
    DETECTION_CONF_STATIC_LIGHT = 0.40
    DETECTION_CONF_FAST_LIGHT = 0.18

# --- 镜头校准 ---
# 工业镜头畸变通常比手机镜头小, 变焦镜头可能有轻微枕形畸变
# 注意：开启畸变校正会降低5-10 FPS，权衡精度和性能
LENS_CORR_ENABLE = False  # 关闭畸变校正，保持高帧率
LENS_CORR_STRENGTH = 0.5  # 校正强度（当ENABLE=True时生效）
...
class CameraConfig:
    ...
    # 补光模式：使用更短曝光，减少运动模糊
    EXPOSURE_US = EXPOSURE_US_WITH_LIGHT if USE_LED_LIGHT else 1200
    ...
    # 补光模式：关闭增益，光照充足
    ANALOG_GAIN_DB = ANALOG_GAIN_DB_WITH_LIGHT if USE_LED_LIGHT else 3
    GAIN_MIN_DB = 0
    GAIN_MAX_DB = 12
...
# 补光模式：图像质量更好，可以提高阈值
DETECTION_CONFIDENCE_STATIC = DETECTION_CONF_STATIC_LIGHT if USE_LED_LIGHT else 0.40   # 球静止/慢速时的阈值
DETECTION_CONFIDENCE_FAST = DETECTION_CONF_FAST_LIGHT if USE_LED_LIGHT else 0.18     # 球快速运动时的最低阈值
...
AXIS_END_PX = (280, 112)
# 实际标尺：25cm管道，24-25cm被合页遮盖，有效范围0-23cm
# 坐标系统：以12.5cm刻度为原点（中心）
AXIS_START_CM = -12.5  # 左端0cm刻度 → 坐标-12.5cm
AXIS_END_CM = 10.5     # 右端23cm刻度 → 坐标+10.5cm（12.5cm刻度为中心0）
...
# --- MaixCAM Pro 补光灯控制 ---
if USE_LED_LIGHT and device_id == "maixcam2":
    try:
        ...
        # 方法1: 检查是否有set_led方法
        if hasattr(cam, 'set_led'):
            cam.set_led(True)
            led_enabled = True
            print("[INFO] MaixCAM Pro 补光灯已开启 (方法1: set_led)")

        # 方法2: 检查是否有led_brightness方法
        elif hasattr(cam, 'led_brightness'):
            cam.led_brightness(100)  # 设置亮度100%
            led_enabled = True
            print("[INFO] MaixCAM Pro 补光灯已开启 (方法2: led_brightness)")

        # 方法3: 尝试常见的GPIO引脚控制补光灯
        else:
            # 尝试可能的LED控制引脚（需要查阅硬件文档确认）
            possible_led_pins = ["GPIOA19", "GPIOA28", "LED", "CAM_LED"]

            for pin_name in possible_led_pins:
                try:
                    led_gpio = gpio.GPIO(pin_name, gpio.Mode.OUT)
                    led_gpio.value(1)  # 开启
                    led_enabled = True
                    print(f"[INFO] MaixCAM Pro 补光灯已开启 (方法3: GPIO {pin_name})")
                    break
                except:
                    continue
        ...
        if not led_enabled:
            # 如果都没成功，说明API或引脚不明确
            print("[WARN] 未找到MaixCAM Pro补光灯控制方法")
            print("[INFO] 建议方案:")
            print("       1. 查阅官方文档: https://wiki.sipeed.com/maixcam")
            print("       2. 或使用外接红外LED补光灯（推荐）")
            print("[INFO] 参数已优化为补光模式（短曝光800μs、低增益0dB）")
        else:
            print(f"[INFO] 补光优化: 曝光{CAM_CFG.EXPOSURE_US}μs, 增益{CAM_CFG.ANALOG_GAIN_DB}dB")
            print(f"[INFO] 检测阈值提高: 静止{DETECTION_CONFIDENCE_STATIC}, 快速{DETECTION_CONFIDENCE_FAST}")

    except Exception as e:
        print(f"[WARN] 补光灯控制失败: {e}")
        print("[INFO] 建议使用外接红外LED补光灯")
        print("[INFO] 继续使用优化的补光参数运行")

elif USE_LED_LIGHT and device_id != "maixcam2":
    print("[INFO] 当前设备非MaixCAM Pro，跳过补光灯控制")
    print("[INFO] 建议使用外接红外LED补光灯")
    print("[INFO] 使用补光优化参数运行")
...
if USE_UART:
    try:
        ...
        print(f"[INFO] 数据格式: ASCII (位置cm + 速度cm/s)")
